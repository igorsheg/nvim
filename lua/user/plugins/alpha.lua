...
return {
  ...
  config = function()
    ...
    local loaded_plugins = string.format("%d/%d plugins", lazy_stats.loaded, lazy_stats.count)

    local version = vim.version() or {}
    local date = os.date "%d-%m-%Y"
    local version_string = string.format("v%s.%d.%d", version.major, version.minor, version.patch)
